...
async def do_muzzle(interOrChannel, user, muzzle_type, target, allowed_phrases, reason, options=None):
    ...
    muzzle_victims[target.mention] = MuzzleVictim(target, muzzle_type=muzzle_type, allowed_phrases=allowed_phrases, muzzled_by=muzzlers[muzzler_mention], guild=interOrChannel.guild, reason_muzzled=reason)
